...
template<class T> void* Ntree<T>::operator new   (size_t size) {
	return _allocator.allocate();
}

template <class T> TAllocationBlock Ntree<T>::_allocator(sizeof(Ntree<T>), 15);

template<class T> void Ntree<T>::operator delete(void *p) {
